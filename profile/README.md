...
| **game_opt_loop** 🔒 | NN 候选的另一条自进化环，覆盖超分与插帧两个赛道，同样通过 `phonefarm` CLI 拿真机数据。2026-09-22 新建，目前是设计文档 + Rust 骨架（本地防爆校验 + dry-run 冒烟跑通），LLM 变异、真机联调、演化代际都还没有真实产出 | Rust |

分工：**物理能力全在 phonefarm，算法循环在 sr_loop 与 game_opt_loop**。二者都不直接碰设备，通过 `phonefarm` 这个独立 CLI 拿真机延迟和游戏画面。knobs 与 refbench 是 2026-09-21 新建的组织内部私有仓库（无公开镜像，仅成员可见），分别给闭环补上「能改什么」（黑盒/灰盒旋钮）与「白盒靶子」（可控 Vulkan 负载），由 `phonefarm/loop_v1/refbench` 驱动，目前刚立项、灰盒层待上机验证。
